...
#DEFINE DATA
x = np.linspace(0, 10,200)					# Return evenly spaced numbers over a specified interval
y  = np.sin(x)							# Used a sin function
y1 = y + np.random.normal(0, 0.5, len(x)) #ADD NOISE

#PLOT 
fig, ax = plt.subplots()
ax.plot(x,y,'-',x,y1,'o')
plt.show()							# plot the x and y
#exit()
...
plt.ylabel('Amplitude (cm)', fontsize=FS)				# define the x label and y label
...
x = np.linspace(0,10,NP)						# Return evenly spaced numbers over a specified interval
...
plt.yticks([-2000,-1000,0,1000,2000],fontsize=FS)				# define x and y labels
