...
#ANN PARAM
layers		=	['whatever',5,5,1] #FIRST NUMBER OVERWRITTEN LATER BY INPUT SHAPE
...
def NN_eval(x,submatrices):
    ...
    for i in range(len(x)):
        n1 = np.matmul(x[i],submatrices[0])+np.transpose(submatrices[1])
        n2 = np.matmul(n1,np.transpose(submatrices[2]))+submatrices[3]
        y.append(n2[0][0])
    return y
